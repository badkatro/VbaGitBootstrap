...
Call doExtraction(Reponame, VBComponentsList, ProjectName)
...
Public Sub doEverything()
   ...
   'doExtraction "vanillacJobject", "advancedcJobject"
    ...
    'doExtraction "FadoSorting", "FadoSorting", "Normal"
    'doExtraction "gscro_lib", "GSCRo_Lib", "gscrolib"
    doExtraction "vbaGitBootstrap_Word", "regXLib", "vbaGitBootstrap_Word"
    
    ' TO DO - BADKATRO - - NEW CODE, TO WRITE, EXPORT FRM AND FRX FILES AND MOD THE EXTRACTED ONES SO AS TO BE ABLE TO IMPORT THEM...
    ' HAVE TO MOD INFO.JSON AS WELL, SO AS TO INCLUDE THEM IN UPLOAD.
    'doFormsExtraction "AssignDocs_vbaGit"
'
'    ' utilities
...
Public Sub doExtraction_AssignDocuments()
   ...
   'doExtraction "vanillacJobject", "advancedcJobject"
    'doExtraction "emptycDataSet", "googleSheets,googleWireExample,oauthexamples,restLibrary"
    'doExtraction "AssignDocuments", "Main,GSCRo_Lib,MouseWheelMod,ColorChooseMod,SettingsFileClass,frmAssign,CalendarForm", "AssignDocs_vbaGit"
    doExtraction "PP_Prepare_HR", "PP_Prepare_HR"
    ...
    If force Or isUndefined(VGSettings) Then
        ...
        With VGSettings.init(Nothing)
            With .add("EXTRACT")
                ...
                .add "TO", "C:/Users/" & Environ("Username") & "/Documents/vbaGit/Extraction/Scripts/"
                '.add "TO", "C:\Users\badkatro\Documents\AssignDocuments\Extraction\Scripts\"
            End With
            With .add("GIT")
                With .add("COMMITTER")
                    .add "name", "Claudiu Margarint"
                    .add "email", "claudiu.margarint@gmail.com"
                End With
                .add "USERAGENT", "badkatro"    ' repo owner
            ...
            With .add("VBA")
                With .add("TYPES")
                    .add "StdModule", 1
                    .add "ClassModule", 2
                    .add "MSForm", 3    ' Experiment, badkatro - try add forms as well, no can do without!
                End With
            End With
        End With
    End If
...
Private Sub doExtraction(Reponame As String, _
    ...
    frmVBAGitExtractor.PrintToOutput "extracting repo " & Reponame & " from project " & project.getObject("project").theProject.name
    DoEvents
    ...
    frmVBAGitExtractor.PrintToOutput "done extracting " & Reponame
...
Private Function createRepos(git As cVbaGit, infos As cJobject) As cJobject
    ...
    For Each info In infos.children
        ...
        If (info.cValue("committedDate") < info.cValue("modifieddate")) Then
            ...
            If (repo Is Nothing) Then
                ...
                frmVBAGitExtractor.PrintToOutput "created repo for " & info.toString("repo")
                DoEvents
    ...
    If (added > 0) Then
        ...
        frmVBAGitExtractor.PrintToOutput "added " & added & " repos"
        DoEvents
...
Private Function writeTheFiles(git As cVbaGit, fileId As String, fileName As String, repo As cJobject) As cJobject
    ...
    frmVBAGitExtractor.PrintToOutput "committing " & fileName & " for " & repo.toString("name")
    frmVBAGitExtractor.Repaint: DoEvents
                ...
                If (d Is Nothing) Then
                    ...
                    Else
                        If (Not warned) Then
                            ...
                            frmVBAGitExtractor.PrintToOutput (matchMod.textKind & " " & matchMod.name & " is skipped - only doing class and stdmodules")
                            DoEvents
                            warned = True
                        End If
                    End If
...
Private Function getVbaAsJobject(Optional optProjectName As String = vbNullString) As cJobject
    Dim project As cJobject, knownProjects As cJobject, _
        module As cJobject, settings As cJobject, wb As Document
...
Private Function getmoduleList(project As cJobject) As cJobject
    ...
    Dim v As VBComponent, vs As VBComponents, wb As Document, vj As cVBAProject
...
Private Function makeReadMe(info As cJobject) As String
    ...
    c.add ("you can see [library and dependency information here](")
    c.add(getVGSettings().toString("FILES.DEPENDENCIES")).addLine(")").addLine ("")
    c.add ("To get started with VBA Git, you can either create a Document with the [code on gitHub](https://github.com/brucemcpherson/VbaGit ""VbaGit repo"")")
    c.add (", or use this premade [VbaBootStrap Document](http://ramblings.mcpher.com/Home/excelquirks/downlable-items/VbaGitBootStrap.xlsm ""VbaBootStrap"")")
    c.add (mdWrap)
...
Private Function makeDependency(project As cJobject, info As cJobject) As String
    ...
    If (info.child("excelReferences").children.Count > 0) Then
        c.add ("####These references were detected in the Document (")
    ...
    Else
        c.add ("####No references were detected in the Document (")
        c.add (project.getObject("project").wBook.name)
